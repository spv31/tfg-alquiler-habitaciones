  ...
  <div class="min-h-screen">
    <div class="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div v-else-if="currentProperty" class="space-y-8">
        ...
        <TenantFormSection
          v-if="currentProperty.data.rental_type === 'full'"
          :property="currentProperty.data"
        />
...
onMounted(async () => {
  try {
    ...
    if (currentProperty.value?.data?.main_image_url) {
      const filename = currentProperty.value.data.main_image_url.split("/").pop() || "";
      propertyImage.value = await propertiesStore.fetchPropertyImageUrl(propertyId, filename);
