<template>
  <div class="min-h-screen max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
    <!-- Header Section -->
    <div class="text-center space-y-2">
      <h1 class="text-3xl font-bold text-gray-900">Panel del Inquilino</h1>
      <p class="text-gray-600 text-lg">Bienvenido a tu espacio de gestión de alquiler</p>
    </div>

    <!-- Main Content -->
    <div class="space-y-8">
      <!-- Property Section -->
      <Card class="rounded-xl shadow-lg hover:shadow-xl transition-shadow duration-300">
        <template #title>
          <div class="flex items-center justify-between p-4">
            <div class="flex items-center gap-3">
              <i class="pi pi-home text-2xl text-primary-600"></i>
              <h2 class="text-xl font-semibold text-gray-900">Tu Alojamiento</h2>
            </div>
            <Button 
              icon="pi pi-comments" 
              label="Abrir chat"
      ...
      <div class="grid gap-6 md:grid-cols-2">
        <!-- Contract Card -->
        <Card class="rounded-xl shadow-sm hover:shadow-md transition-shadow duration-300">
          <template #title>
          ...
          <template #content>
            <ContractSection
              ...
              :rentable-type="assignedRentable?.type"
              class="p-4"
            />
          </template>
        </Card>
        ...
        <Card class="rounded-xl shadow-sm hover:shadow-md transition-shadow duration-300">
          <template #title>
            <div class="flex items-center gap-3 p-4">
              <i class="pi pi-credit-card text-2xl text-primary-600"></i>
              ...
              <div v-if="currentContract" class="space-y-6">
                ...
                <div class="space-y-4">
                  <div class="flex justify-between items-center">
                    ...
                    <span class="font-semibold text-lg text-gray-900">${{ currentContract.rent_amount }}</span>
                  </div>
                  <div class="flex justify-between items-center">
                    <span class="text-gray-600">Próximo vencimiento:</span>
                    <span class="font-medium text-gray-900">05 {{ nextMonth }}</span>
                  </div>
                </div>
...
const { currentContract, assignedRentable } = storeToRefs(tenantStore)
...
onMounted(async () => {
  if (!assignedRentable.value || !currentContract.value) {
    await tenantStore.fetchTenantData().catch(console.error)
  }
})
</script>